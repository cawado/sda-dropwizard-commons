...
public class OpaRuleIT {
  ...
  public void shouldReturnResponseOnMatch() {
    // given
    OPA_RULE.mock(onRequest().withMethod(method).withPath(path).withJwt(jwt).allow());
    // when
    Response response = requestMock(requestWithJwt(jwt, method, path));
    // then
    assertThat(response.getStatus()).isEqualTo(SC_OK);
    assertThat(response.readEntity(OpaResponse.class).isAllow()).isTrue();
    OPA_RULE.verify(1, onRequest().withMethod(method).withPath(path).withJwt(jwt));
  }
  ...
  public void shouldReturnDifferentResponsesInOneTest() {
    ...
    OPA_RULE.mock(onRequest().withMethod("POST").withPath("pathD").withJwt(null).deny());
    // when
    ...
    OPA_RULE.verify(1, onRequest().withMethod("POST").withPath("pathC").withJwt(jwt));
    OPA_RULE.verify(1, onRequest().withMethod("POST").withPath("pathD").withJwt(null));
  }
